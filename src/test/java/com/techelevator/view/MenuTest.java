...
public class MenuTest {
    ...
    //Test to verify that the balance message is displayed in the purchase menu
    @Test
    public void displays_a_list_of_menu_options_and_displays_balance_message() {
        Object[] options = new Object[]{Integer.valueOf(3), "Blind", "Mice"};
        Transactions transactions = new Transactions();

        Menu menu = getMenuForTesting();

        menu.getChoiceFromOptions(options, transactions);

        String expected = System.lineSeparator() + "1) " + options[0].toString() + System.lineSeparator() + "2) " + options[1].toString() + System.lineSeparator() + "3) "
                + options[2].toString() + System.lineSeparator() + System.lineSeparator()
                + transactions.balanceMessage() + System.lineSeparator() + System.lineSeparator() + "Please choose an option >>> ";
        Assert.assertEquals(expected, output.toString());
    }
